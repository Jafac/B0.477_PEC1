...
setwd("C:/Users/03743985/Documents/Repositorios_git/B0.477_PEC1/")
```{r}
setwd("C:\\Users\\acamp\\OneDrive\\Documentos\\UOC\\proyectos_git\\B0.477_PEC1")
```
...
El apartado "Non-/sparse entries" define datos no-dispersos/datos dispersos, por lo tanto cuando en mat_acq dice que su valor es 17687/49345 significa que en toda la matriz hay 17687 datos distintos que cero y 49345 datos que son cero. 

Según la definición de "sparse matrix" en la wikipedia, se define el concepto "sparsity" como el número de datos que son cero entre el total de los elementos, esto es 49345/(49345+17687)

```{r}
sparsity.ejercicio1 <- 100 * (49345 / (49345+17687))
#El aparado de sparsity vienen redondeado
round.sparsity.ejercicio1 <- round(sparsity.ejercicio1)
mat_acq$mat
round.sparsity.ejercicio1
```
Vemos que los terminos coinciden.

Por tanto, los campos "Non-/sparse entries" y "Sparsity" nos dan una idea de cómo de dispersa o densa (opuesto a dispersa) es una matriz.
...
La matriz de "earn" la podemos encontrar en `mat[[2]]$mat`.

Seguidamente visualizaremos los 10 primeros terminos y los 10 primeros documentos de esta temática:
```{r}
#frecuencia 10 primeros terminos
inspect(mat[[2]]$mat[1:10,])

#frecuencia de los 10 primeros documentos
inspect(mat[[2]]$mat[,1:10])
```

Ahora visualizaremos aquellas palabras con frecuencia menor a 1000 y mayor a 10 (10 < f < 1000):
```{r}
findFreqTerms(mat[[2]]$mat, lowfreq = 10, highfreq = 1000)
```
...
Reusamos el 'fdata' ya construido (concatenacion de las matrices para 'acq' y 'earn') para generar la grafica:
```{r}
colnames(fdata)
```

Para cambiar las barras separadas por las barras apiladas hay que cambiar el parametro 'position' de la función 'geom_bar': de 'position_dodge()' pasa a ser 'position_stack()'.
https://www.rdocumentation.org/packages/ggplot2/versions/3.2.1/topics/geom_bar

Para cambiar la posición es suficiente con añadir la función cood_flip() para que invierta los ejes y asi ver el grafico en horizontal.
https://www.rdocumentation.org/packages/ggplot2/versions/3.2.1/topics/coord_flip
...
Las barras bicolor pertenecen a terminos mayoritarios (frecuencia > 500) en las dos tematicas, por tanto, cuantos mas graficos bicolor mas similares son los temas. ESo puede implicar la diferenciación puede ser mas costosa.

En el grafico podemos ver 21 palabras de tematica "acq" y 29 palabras "earn". Hay 8 barras bicolores, que representan aproximadamente el 30% de cada tematica. Por tanto, yo diria que hay bastantes en comun y que las tematicas son relativamente similares.
...
```
El ratio1 es la exactitud (TP / (TP + FP))
El ratio2 es la especificidad (TN/N) donde N = FP + TN (todos los negativos "reales")

Los positivos "reales" (P) se calculan como P = TP + FN
Los negativos "reales" (N) se calculan como N = TN + FP

La precision se calcula como Precisión = (TP + TN) / (P + N)
```{r}
P <- sum(conf.mat[,1])
N <- sum(conf.mat[,2])
TP <- conf.mat[1,1]
...
El algoritmo knn usa los k vecinos más próximos para decidir su predicción. Entonces el número de vecinos que se use, puede influir en el resultado.

Para ver cómo influye el número de vecinos en este problema en concreto veremos una gráfica que nos indique la precisión en función del número de vecinos:

(usaremos números impares para evitar empates)

```{r,eval=TRUE,echo=TRUE,warning=FALSE, message=FALSE}
set.seed(111)

precision_knn <- function(k_number) {
  set.seed(111)
   knn.pred <- knn(pila.nl[entrena.idx, ], pila.nl[test.idx, ], tema[entrena.idx], k=k_number) 
   conf.mat <- table("Predicción" = knn.pred,"Real" = tema[test.idx])
   precision <- (conf.mat[1,1] + conf.mat[2,2]) / (sum(conf.mat))
   return(precision)
}

x <- as.list(seq(1,50,2))
y <- lapply(x, precision_knn)
plot(x, y)
```

Casi todos tienen una precisión entre el 96.8% y el 97.6%. Podemos decir que la mayoría son bastante buenos. Finalmente veremos la matriz de confusión para 17 vecinos (mejor precisión en el gráfico):

```{r}
set.seed(111)
knn17.pred <- knn(pila.nl[entrena.idx, ], pila.nl[test.idx, ], tema[entrena.idx], k=17) 
conf17.mat <- table("Predicción" = knn17.pred,"Real" = tema[test.idx])
conf17.mat

#Comparamos con la original
conf.mat
```

Para 17 vecinos, predice peor la temática "acq".
Para 1 vecino, predice peor la temática "earn"
...
Repetiremos todo lo que hemos hecho anteriormente para preparar los datos para usar el algoritmo k-NN:

```{r,eval=TRUE,echo=TRUE,warning=FALSE, message=FALSE}
# Seleccionamos dos temáticas: interest y trade
data2<-data[which(data$V1 %in% c("interest","trade")),]

# Creación del corpus de la temática interest
## Seleccionamos la temática
data_int<-data2[(data2$V1=="interest"),]
## Construimos el corpus
source <- VectorSource(data_int$V2)
corpus1 <- Corpus(source)
## Acondicionamos el corpus
corpus1 <- tm_map(corpus1, content_transformer(tolower))
corpus1 <- tm_map(corpus1, removeNumbers)
...
## Seleccionamos la temática
data_tra<-data2[(data2$V1=="trade"),]
## Construimos el corpus
source <- VectorSource(data_tra$V2)
corpus2 <- Corpus(source)
## Acondicionamos el corpus
corpus2 <- tm_map(corpus2, content_transformer(tolower))
corpus2 <- tm_map(corpus2, removeNumbers)
...
mat_int <- TermDocumentMatrix(corpus1)
## Controlamos la dispersión (Sparsity): Número de celdas igual a cero respecto al total.
mat_int<- removeSparseTerms(mat_int,  0.85)
mat_int<-list(name="interest",mat=mat_int)

# Construimos la matrix de documentos de la temática trade
mat_tra <- TermDocumentMatrix(corpus2)
mat_tra<- removeSparseTerms(mat_tra,  0.85)
mat_tra<-list(name="trade",mat=mat_tra)

# Juntamos ambas matrices de términos en una misma lista
mat<-list(mat_int, mat_tra)
str(mat)


# Creación de un data.frame apto para K-NN
# Para interest
s.mat_int <- t(data.matrix(mat[[1]]$mat))
# La convertimos en data.frame que vendría a ser como un formato excel (filas, columnas y celdas con valores)  
# En este data.frame, tenemos que cada fila es un documento, cada columna una palabra y las celdas contienen la frecuencia en que cada palabra aparece en cada documento.
s.df_int <- as.data.frame(s.mat_int, stringsAsFactors = FALSE)
# En la última columna colocaremos el Tema de cada documento tdm[["name"]. Para ello usaremos dos funciones cbind() y rep()  

# Recordemos que en la lista TDM habíamos almacenado el tema en el valor "name"
# Mediante la función rep() repetiremos el tema del documento tantas veces como filas hay en el data.frame 
Tema <- rep(mat[[1]]$name, nrow(s.df_int))
s.df_int<-cbind(s.df_int,Tema)

# Para trade
s.mat_tra <- t(data.matrix(mat[[2]]$mat))
# La convertimos en data.frame que vendría a ser como un formato excel (filas, columnas y celdas con valores)  
# En este data.frame , tenemos que cada fila es un documento, cada columna una palabra y las celdas contienen la frecuencia en que cada palabra aparece en cada documento.
s.df_tra <- as.data.frame(s.mat_tra, stringsAsFactors = FALSE)
# En la última columna colocaremos el Tema de cada documento tdm[["name"]. Para ello usaremos dos funciones cbind() y rep()  

# Recordemos que en la lista TDM habíamos almacenado el tema en el valor "name"
# Mediante la función rep() repetiremos el tema del documento tantas veces como filas hay en el data.frame 
Tema <- rep(mat[[2]]$name, nrow(s.df_tra))
s.df_tra<-cbind(s.df_tra,Tema)

# Utilizamos la función rbind.fill() para concatenar las filas de dos data frame con distinta dimensión y pone NA en las casillas donde no hay información.
pila <-rbind.fill(s.df_int, s.df_tra)
pila[is.na(pila)] <- 0
...
Ejecutamos el algoritmo y vemos su matriz de confusión
...
Según el libro "Minería de datos: modelos y algoritmos" que hay en los recursos de la asignatura, el algoritmo k-NN realiza la clasificación de la siguiente forma:

1. Se fija un valor k.
2. Se calculan las k instancias más cercanas en función de la métrica de similitud utilizada.
3. Se asigna la instancia a la clase más frecuente entre las k instancias más cercanas.
...
Para la función 'knn' no existe una forma directa, pero se pueden llegar a usar otras librerías para conseguirlo (o incluso generar nosotros el algoritmo) como, por ejemplo: https://www.rdocumentation.org/packages/KODAMA/versions/0.0.1/topics/knn.dist

Una distancia que podría mejorar los resultados es la distancia de Mahalanobis (https://en.wikipedia.org/wiki/Mahalanobis_distance) porque es invariante a la escala y además tiene en cuenta la correlación de las variables. Esto implica que las similitudes serán más acertadas y por tanto las predicciones también lo serán.
